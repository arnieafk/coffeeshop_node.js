...
async function viewMenu(req, res) {
  try {
    ...
    res.render('customer/menu', { products, cart: getCart(req) });
  } catch (error) {
    console.error('Error fetching products:', error);
    res.status(500).render('partials/error', { message: 'Failed to load menu' });
  }
}
...
async function addToCart(req, res) {
  try {
    ...
    if (!product) return res.redirect('/customer/menu');
    ...
    res.status(500).render('partials/error', { message: 'Failed to add item to cart' });
  }
}
...
function removeFromCart(req, res) {
  const cart = getCart(req).filter((item) => item.id !== Number(req.params.id));
  req.session.cart = cart;
